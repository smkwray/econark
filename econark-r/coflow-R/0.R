...
parse_args <- function(argv) {
  out <- list(
    config = "config_coflow_poverty_consumption_interp.R",
    stage = "all",
    seed = NULL,
    tz = NULL,
    locale = NULL
...
apply_run_context <- function(args) {
  seed_raw <- .pick_context_value(args$seed, c("COFLOW_RUN_SEED", "ECONARK_RUN_SEED"), "20260225")
  tz <- .pick_context_value(args$tz, c("COFLOW_RUN_TZ", "ECONARK_RUN_TZ"), "UTC")
  locale <- .pick_context_value(args$locale, c("COFLOW_RUN_LOCALE", "ECONARK_RUN_LOCALE"), "C")
...
cat(sprintf("[coflow-R] run_context seed=%d tz=%s locale=%s\n", ctx$seed, ctx$tz, ctx$locale))
config_path <- args$config

# Accept either absolute paths, cwd-relative paths, or module-relative paths.
...
run_launcher(config_path, stage = args$stage, context = ctx)
