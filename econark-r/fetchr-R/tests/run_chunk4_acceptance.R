...
main <- function() {
  ...
  steps <- list(
    ...
    list(step_id = "chunk4_contract", label = "panel/table/scenario family checks", command = "Rscript", args = c("tests/test_panel_table_scenario_outputs.R"))
  )

  rows <- lapply(steps, function(st) run_step(st$step_id, st$label, st$command, st$args, wd = root))
  summary_df <- do.call(rbind, rows)

  cat("[SUMMARY] fetchr-R chunk4 acceptance\n")
  print(summary_df, row.names = FALSE)

  failed <- summary_df$status == "fail"
  if (any(failed)) {
    stop(sprintf("Chunk4 acceptance failed (%d step(s))", sum(failed)), call. = FALSE)
  }

  cat(sprintf("[PASS] fetchr-R chunk4 acceptance steps=%d\n", nrow(summary_df)))
}
