    ...
    *,
    ...
    for attempt in range(attempts):
        try:
            ...
            should_retry = int(resp.status_code) in set(retry_statuses) and attempt + 1 < attempts
            if should_retry:
                resp.close()
...
def fetch_qwi_api(spec: Dict[str, Any], cfg: Dict[str, Any]) -> pd.Series:
    ...
    for year in range(start_year, end_year + 1):
        ...
        for payload in requests_to_make:
            resp = requests.get(base_url, params=payload, headers={"User-Agent": ua}, timeout=timeout)
...
def _download_text_with_curl_fallback(url: str, *, timeout: int, user_agent: str, use_curl_fallback: bool) -> str:
    try:
...
def fetch_ui_eta203(spec: Dict[str, Any], cfg: Dict[str, Any]) -> pd.Series:
    ...
    if input_src:
        ...
        if date_col in df.columns and value_col in df.columns:
            ...
            return normalize_series(out, name=series_name)
    else:
...
def _download_cex_bytes(year: int, *, timeout: int, user_agent: str) -> tuple[bytes, str]:
    ...
    if 2004 <= year <= 2011:
        ...
        resp2 = requests.get(alt_url, headers={"User-Agent": user_agent}, timeout=timeout)
...
def fetch_bls_cex_share(spec: Dict[str, Any], cfg: Dict[str, Any]) -> pd.Series:
    ...
    for year in range(start_year, end_year + 1):
        try:
            blob, ext = _download_cex_bytes(year, timeout=timeout, user_agent=ua)
            if ext == "txt":
...
def _fetch_treasury_mspd_api(spec: Dict[str, Any], cfg: Dict[str, Any]) -> pd.DataFrame:
    base_url = str(
        spec.get(
            "base_url",
...
def _load_treasury_ledger(spec: Dict[str, Any], cfg: Dict[str, Any]) -> pd.DataFrame:
    input_src = _resolve_input_source(spec, cfg)
    if input_src:
        raw = pd.read_csv(input_src, low_memory=False)
    else:
        raw = _fetch_treasury_mspd_api(spec, cfg)
...
def fetch_treasury_mspd(spec: Dict[str, Any], cfg: Dict[str, Any]) -> pd.Series:
    ...
    if metrics is None:
        ledger = _load_treasury_ledger(spec, cfg)
