"""
Example alternate DFLMX profile configuration.

This is a generic template for creating alternative analysis profiles.

How to use:
1. Copy this file to `config_dflmx_alt.py` only if you need a secondary profile.
2. Keep `config_dflmx.py` as the default runtime config for `launcher.py`.
3. Use this profile for controlled A/B or robustness runs.

Profile guidance:
- Keep this file structurally identical to the default config.
- Only change the parameters you want to compare (k, lags, horizons, worker caps).
- Keep data paths shared unless you intentionally isolate outputs.
- Customize the same fields as config_dflmx.py (outcomes, hypotheses, treatments).
"""
...
# This alternate profile keeps the same input/output contract as config_dflmx.
# Prefer changing analysis knobs rather than path conventions for clean diffs.
